...
import com.skyecodes.karith.api.mathContext
...
class KthTokenizerImplTest {
    private val ctx = mathContext { withPowerOperator() } as KthContextImpl

    @Test
    fun testTokenize_ShouldReturnCorrectTokens(args: TokensArguments = burstValues(
        ...
        assertContentEquals(output, ctx.tokenizer.tokenize(input, declaredVars).first.map { it.value })
    }

    @Test
    fun testTokenize_ShouldReturnCorrectVariables(args: VariablesArguments = burstValues(
        ...
        assertContentEquals(output, ctx.tokenizer.tokenize(input, null).second.asIterable())
    }

    @Test
    fun testTokenize_ShouldThrowException_WhenUnknownToken() {
        ctx.tokenizer.tokenize("3+2", emptyList())
        ctx.tokenizer.tokenize("3+2a", null)
        assertFailsWith<KthIllegalTokenException> { ctx.tokenizer.tokenize("3+2a", emptyList()) }
    }
